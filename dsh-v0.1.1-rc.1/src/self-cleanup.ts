 ...
 *
 * 纪律：
 *  - 只删插件可再生的运行时状态（runtime-config / deliver-target / sync-index /
 *    pending-deliveries / history / plugin.log / session-configs），
 *    保留用户资产（recordings 录音归档、whisper/qwen 模型、sessions 会话、notes 笔记）；
 *  - 宽限期：连续 GRACE_CHECKS 次检测到已移除才触发，避免「remove 后立刻 add
 *    重装」误删；更新（dsh plugin update）不会让 manifest 短暂丢失包名；
 *  - 拿不到 profile 目录（普通 DSH 无 desktopProfiles）时不启用，回退手动脚本。
 */
...
const OWNED_DIRS = ['session-configs']
const OWNED_FILES = [
  'runtime-config.json',
  'deliver-target.json',
  'sync-index.json',
