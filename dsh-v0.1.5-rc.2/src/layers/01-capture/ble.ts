...
export interface BleFileEntry {
  ...
  size: number
  name: string
  /** 列表里的原始 28B 条目（base64）。删除的「格式 A」要原样回传 */
  raw?: string
}
 ...
 * 从设备文件名解析**录制时间**。
 *
 * 录音卡给文件起的名字本身就是时间：`note20260820-235231.opus` = 2026-08-20 23:52:31。
 * 这是录制时间的唯一来源——同步时刻只表示「什么时候把它拷下来的」，
 * 拿同步时刻当录制时间会让一卡旧录音全变成「刚刚」，排序与归档时间戳全错。
 *
 * 认不出返回 null，调用方回退到当前时间。
 */
export function deviceFileTime(name: string): Date | null {
  const m = /(?:^|\D)(\d{4})(\d{2})(\d{2})-(\d{2})(\d{2})(\d{2})(?:\D|$)/.exec(name ?? '')
  if (!m) return null
  const [, y, mo, d, h, mi, s] = m
  const dt = new Date(Number(y), Number(mo) - 1, Number(d), Number(h), Number(mi), Number(s))
  // 逐字段回读：挡掉 20261345-996161 这种能被 Date 归一化成合法值的假时间
  if (
    dt.getFullYear() !== Number(y) || dt.getMonth() !== Number(mo) - 1 || dt.getDate() !== Number(d) ||
    dt.getHours() !== Number(h) || dt.getMinutes() !== Number(mi) || dt.getSeconds() !== Number(s)
  ) {
    return null
  }
  return dt
...
export class BleCentral {
  ...
  async download(
    name: string,
    opts: { chunkBytes?: number; chunkTime?: number; prefer?: 'opus' | 'wav' } = {},
  ): Promise<{ name: string; ext: 'wav' | 'opus'; size: number; data: string; chunks?: number }> {
    const params: Record<string, unknown> = { name }
    if (opts.chunkBytes) params.chunk_bytes = opts.chunkBytes
    if (opts.chunkTime) params.chunk_time = opts.chunkTime
    // .opus 是设备原生存储格式（≈2 KB/s），.wav 是设备转码产物（32 KB/s，体积 16 倍）
  ...
  async deleteFile(name: string, time: number, size: number, raw?: string): Promise<Record<string, unknown>> {
    // raw = 列表里的原始 28B 条目：删除的「格式 A」必须原样回传，
    // 不能自己重新拼（拼出来的字节序/截断规则可能与固件不一致）
    const params: Record<string, unknown> = { name, time, size }
    if (raw) params.raw = raw
    return await this.request('delete', params, 60000)
  }
